...
public class ProjectServiceImpl implements ProjectService {
    ...
    public ProjectResponseDto createProject(ProjectRequestDto requestDto) {
        ...
        // Validate that each milestone has at least one associated department
        for (ProductMilestoneMap milestone : milestones) {
            if (milestone.getMilestone().getDepartments().isEmpty()) {
                        ...
                        if (assignment.getAssignedUser() == null && !map.isAutoGenerated()) {
                            try {
                                ...
                                logger.debug("Assigned user {} (ID: {}) to Certification milestone for project ID: {}",
                                        assignedUser.getFullName(), assignedUser.getId(), project.getId());
                        ...
                        if (assignment.getAssignedUser() == null && !map.isAutoGenerated()) {
                            try {
                                ...
                                logger.debug("Assigned user {} (ID: {}) to milestone {} for project ID: {}",
                                        assignedUser.getFullName(), assignedUser.getId(),
    ...
    private User assignMilestoneUser(ProductMilestoneMap milestone) {
        ...
        if (!mappings.isEmpty()) {
            logger.debug("UserProductMap entries for product ID: {}: {}",
                    ...
                    mappings.stream()
                            .map(m -> String.format("User: %s (ID: %d, Rating: %s, Deleted: %b)",
                                    m.getUser().getFullName(), m.getUser().getId(),
                                    m.getRating() != null ? m.getRating() : "null", m.getUser().isDeleted()))
                            .collect(Collectors.joining(", ")));
        }
        ...
        if (bestUserOpt.isPresent()) {
            ...
            double rating = mappings.stream()
                    .filter(m -> m.getUser().getId().equals(bestUser.getId()))
                    .findFirst()
                    .map(m -> m.getRating() != null ? m.getRating() : 0.0)
                    .orElse(0.0);
            ...
            if (rating > 0) {
                logger.info("Assigned user: {} (ID: {}, Rating: {}) for milestone: {}",
                        bestUser.getFullName(), bestUser.getId(), rating, milestone.getMilestone().getName());
                return bestUser;
            } else {
                User manager = bestUser.getManager();
                if (manager != null && isUserAvailable(manager)) {
                    logger.info("No user with positive rating found, assigned manager: {} (ID: {}) for milestone: {}",
                            manager.getFullName(), manager.getId(), milestone.getMilestone().getName());
                    return manager;
                } else {
                    logger.debug("No available manager for user: {} (ID: {}) for milestone: {}",
                            bestUser.getFullName(), bestUser.getId(), milestone.getMilestone().getName());
                    return assignAdmin(milestone);
                }
            }
        }
