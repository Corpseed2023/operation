    ...
    private void updateMilestoneStatuses(Project project, Long updatedById) {
        ...
        for (ProjectMilestoneAssignment assignment : assignments) {
            if ("LOCKED".equals(assignment.getStatus())) {
                ...
                // Special condition for Certification milestone (milestone_id = 5)
                if (map.getMilestone().getId() == 5 || "Certification".equalsIgnoreCase(map.getMilestone().getName())) {
                    ...
                    if (dueAmount == 0) {
                        ...
                        projectMilestoneAssignmentRepository.save(assignment);
                        logger.debug("Unlocked Certification milestone for project ID: {}, milestone: {}", project.getId(), map.getMilestone().getName());
                    }
                } else {
                    ...
                    if (allPreviousUnlocked && paidAmount >= requiredPayment) {
                        ...
                        projectMilestoneAssignmentRepository.save(assignment);
                        logger.debug("Unlocked milestone for project ID: {}, milestone: {}, cumulative payment: {}",
