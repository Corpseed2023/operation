...
public class ProjectServiceImpl implements ProjectService {
    ...
    public ProjectResponseDto createProject(ProjectRequestDto requestDto) {
        ...
        // Removed department check here, as it's handled in AutoAssignmentServiceImpl
        /* for (ProductMilestoneMap milestone : milestones) {
            if (!milestone.isAutoGenerated() && milestone.getMilestone().getDepartments().isEmpty()) {
                throw new ValidationException("Milestone " + milestone.getMilestone().getName() + " has no associated departments", "ERR_NO_DEPARTMENTS_FOR_MILESTONE");
            }
        } */
        ...
        for (ProductMilestoneMap milestone : milestones) {
            ...
            if (!milestone.isAutoGenerated()) {
                logger.info("Attempting to assign user for milestone: {} in project: {}", milestone.getMilestone().getName(),
                ...
                if (result != null && result.getUser() != null) {
                    ...
                    System.out.println("Assigned milestone " + milestone.getMilestone().getName() + " (project ID: "
                            + project.getId() + ") to user ID: " + result.getUser().getId() + " (Name: " + result.getUser().getFullName()
                            + ") with reason: " + result.getReason());
                } else {
                    assignment.setAssignedUser(null);
                    assignment.setStatusReason(result.getReason());
                    // Fix 1: Ensure QUEUED status is preserved
